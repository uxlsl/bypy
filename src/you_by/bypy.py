...
or (sys.version_info[0] == 3 and sys.version_info[1] < 3):
	...
	sys.exit(constants.EIncorrectPythonVersion)
import io
import locale
import codecs
...
class cached(object):
	...
	verbose = False
	debug = False
	hashcachepath = config.HashCachePath
	cache = {}
	cacheloaded = False
	dirty = False
...
class ByPy(object):
	...
	def migratesettings():
		...
		filesToMove = [
			[config.OldTokenFilePath, config.TokenFilePath],
			[config.OldPicklePath, config.PicklePath]
		]

		result = makedir(config.ConfigDir, 0o700) and result # make it secretive
		# this directory must exist
		if result != constants.ENoError:
			perr("Fail to create config directory '{}'".format(config.ConfigDir))
			return result
		...
		if os.path.exists(config.PicklePath):
			oldcache = {}
			try:
				with io.open(config.PicklePath, 'rb') as f:
				...
				"has been merged to '{}'".format(config.PicklePath, config.HashCachePath))
				mergedfile = config.PicklePath + '.merged'
				ok = movefile(config.PicklePath, mergedfile)
				if ok == constants.ENoError:
				...
				perr("{} invalid Pickle (old format hash cache) file '{}' to '{}'".format(
					"Moved" if ok == constants.ENoError else "Failed to move",
	...
	def __init__(self,
		slice_size = config.DefaultSliceSize,
		dl_chunk_size = config.DefaultDlChunkSize,
		verify = True,
		...
		self.__tokenpath = configdir + os.sep + config.TokenFileName
		self.__settingpath = configdir + os.sep + config.SettingFileName
		self.__setting = {}
		...
		self.__hashcachepath = configdir + os.sep + config.HashCacheFileName
		cached.hashcachepath = self.__hashcachepath
		self.__certspath = configdir + os.sep + config.ByPyCertsFileName
		...
		if mirror and mirror.lower() != PcsDomain:
			pcsurl = 'https://' + mirror + config.RestApiPath
			cpcsurl = pcsurl
			dpcsurl = pcsurl
...
"you are not going to send sensitive data to the cloud plainly right?")
		...
		try:
			...
			if sc == requests.codes.ok or sc == 206: # 206 Partial Content
				...
				if ec == 111 or ec == 110 or ec == 6: # and sc == 401:
					...
					if constants.ENoError == self.__refresh_token(): # refresh the token and re-request
	...
	def __request(self, url, pars, act, method, actargs = None, retry = True, addtoken = True, dumpex = True, **kwargs):
		...
		if 'headers' not in kwnew:
			kwnew['headers'] = { 'User-Agent': config.UserAgent }
		...
		while True:
			...
			if result == ERequestFailed:
				if i < tries:
					...
					delay = config.RetryDelayInSec * i
...
'''
	...
	def __upload_file_slices(self, localpath, remotepath, ondup = 'overwrite'):
		...
		if ec != constants.ENoError:
			...
			if ec == constants.ENoError or ec == IESuperfileCreationFailed:
	...
	def __walk_upload(self, localpath, remotepath, ondup, walk):
		...
		for name in filenames:
			...
			if subresult == constants.ENoError: # same-name remote file exists
				self.__isrev = True
				if constants.ENoError == self.__verify_current_file(self.__remote_json, False):
			...
			if upload:
				...
				if fileresult != constants.ENoError:
	...
	def __upload_dir(self, localpath, remotepath, ondup = 'overwrite'):
		...
		for walk in self.__walk_normal_file(localpath):
			...
			if thisresult != constants.ENoError:
				result = thisresult
    ...
    ondup - what to do upon duplication ('overwrite' or 'newcopy'), default: 'overwrite'
		...
		if os.path.isfile(lpath):
			...
			if rpath[-1] == '/': # user intends to upload to this DIR
				...
				if subresult == constants.ENoError: # remote path exists, check is dir or file
  ...
  md5s - MD5 digests of the slices, can be:
	...
	def __downchunks(self, rfile, start):
		...
		while True:
			...
			if subresult != constants.ENoError:
				return subresult
...
#					self.__isrev = False
	...
	def __get_cdl_dest(self, source_url, save_path):
		...
		if rpath == AppPcsPath \
			or (constants.ENoError == self.__get_file_info(rpath) \
  ...
  task_id - id of the task to be canceled.
	...
	def __share_local_file(self, lpath, rpath, fast):
		...
		if not self.__rapiduploaded:
			...
			while i < ShareRapidUploadRetries:
				...
				if result == constants.ENoError: # or result == IEMD5NotFound: # retrying if MD5 not found _may_ make the file available?
					break;
				else:
	...
	def __ok_to_use_remote_temp_dir(self):
		...
		if fir == constants.ENoError: # path exists
			msg = '''
...
def onexit(retcode = constants.ENoError):
...
def getparser():
	...
	parser.add_argument("-s", "--slice", dest="slice", default=config.DefaultSliceSize, help="size of file upload slice (can use '1024', '2k', '3MB', etc) [default: {} MB]".format(config.DefaultSliceInMB))
	parser.add_argument("--chunk", dest="chunk", default=config.DefaultDlChunkSize, help="size of file download chunk (can use '1024', '2k', '3MB', etc) [default: {} MB]".format(config.DefaultDlChunkSize // OneM))
	...
	parser.add_argument(config.DisableSslCheckOption, dest="checkssl", action="store_false", help="DON'T verify host SSL cerificate")
	parser.add_argument(config.CaCertsOption, dest="cacerts", help="Specify the path for CA Bundle [default: %(default)s]")
	...
	# action
	parser.add_argument(config.CleanOptionShort, config.CleanOptionLong, dest="clean", action="count", default=0, help="1: clean settings (remove the token file) 2: clean settings and hash cache [default: %(default)s]")
...
def clean_prog_files(cleanlevel, verbose, tokenpath = config.TokenFilePath):
...
def main(argv=None): # IGNORE:C0111
	...
	try:
		...
		if os.path.exists(config.HashCachePath):
			cachesize = getfilesize(config.HashCachePath)
			if cachesize > 10 * OneM or cachesize == -1:
				pr((
				   ...
				   "*** WARNING ***\n\n\n").format(config.HashCachePath, human_size(cachesize)))
